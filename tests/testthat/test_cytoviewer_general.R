test_that("cytoviewer: app testing works", {
  ...
  expect_silent(app <- cytoviewer(image = pancreasImages, mask = pancreasMasks, object = pancreasSCE, img_id = "ImageNb", cell_id = "CellNb"))
  ...
  expect_silent(app_2 <- cytoviewer(image = pancreasImages, mask = pancreasMasks, img_id = "ImageNb"))
  expect_equal(class(app_2), "shiny.appobj")
  expect_equal(class(app_2$serverFuncSource()), "function")
  
  expect_silent(app_3 <- cytoviewer(mask = pancreasMasks, object = pancreasSCE, img_id = "ImageNb", cell_id = "CellNb"))
