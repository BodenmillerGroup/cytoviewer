...
#'cytometry and other highly multiplexed imaging techniques. The \code{cytoviewer} 
#'interface is divided into image-level (Composite and Channels), cell-level (Masks)
#'and pixel-level (Graph) visualization. It allows users to overlay individual 
#'images with segmentation masks, integrates well with \code{SingleCellExperiment} 
#'and \code{SpatialExperiment} objects for metadata visualization and supports 
#'image downloads.
#'
...
#' cell-level visualization, points-level visualization, overlaying images 
...
#' points-level visualization  as well as metadata visualization is possible.
#'    
...
#' 
#' \code{\link[imcRtools]{plotSpatial}} for the function underlying 
